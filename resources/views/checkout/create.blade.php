...
@section('title', __('Passer commande'))

@section('content')
    <div class="page-head">
        <div class="page-titles">
            <h1>{{ __('Finaliser ma commande') }}</h1>
            <p>{{ __("Renseignez l'adresse de livraison et les informations du destinataire.") }}</p>
        </div>
        <div class="page-actions">
            <a href="{{ route('panier.index') }}" class="btn btn-ghost"><x-icon name="arrow-left" size="16" /> {{ __('Retour au panier') }}</a>
        </div>
    </div>
    ...
    @if ($errors->any())
        <div class="alert alert-err">
            ...
            <div>
                {{ __('Merci de corriger les champs suivants :') }}
                <ul>
    ...
    <form method="POST" action="{{ route('checkout.store') }}">
        ...
        <div class="cart-grid">
            <div class="stack" style="gap: 18px;">
                <div class="form-card">
                    <h3 style="margin: 0 0 18px; font-size: 16px; font-weight: 800;">{{ __('Vos coordonnées') }}</h3>
                    <div class="form-grid">
                        <div class="field">
                        ...
                        <div class="field">
                            <label class="label">{{ __('Téléphone') }} <span class="req">*</span></label>
                        ...
                        <div class="field">
                            <label class="label">{{ __('Email (optionnel)') }}</label>
                ...
                <div class="form-card">
                    <h3 style="margin: 0 0 18px; font-size: 16px; font-weight: 800;">{{ __('Livraison') }}</h3>
                    <div class="form-grid">
                        <div class="field col-span-2">
                            <label class="label">{{ __('Adresse de livraison') }} <span class="req">*</span></label>
                            <textarea name="adresse_livraison" class="textarea @error('adresse_livraison') has-err @enderror" required placeholder="{{ __('N°, rue, quartier, ville…') }}">{{ old('adresse_livraison') }}</textarea>
                            @error('adresse_livraison') <span class="field-err">{{ $message }}</span> @enderror
                        </div>
                        <div class="field">
                            <label class="label">{{ __('Nom du destinataire') }} <span class="req">*</span></label>
                        ...
                        <div class="field">
                            ...
                            <span class="field-hint">{{ __('Servira aussi à suivre la commande.') }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="summary">
                <div class="card card-pad">
                    <h3 style="margin: 0 0 14px; font-size: 16px; font-weight: 800;">{{ __('Votre commande') }}</h3>
                    ...
                    <button type="submit" class="btn btn-primary btn-block" style="margin-top: 16px;">
                        <x-icon name="check" size="17" stroke="2.2" /> {{ __('Confirmer la commande') }}
                    </button>
                    <p class="muted" style="font-size: 11.5px; text-align: center; margin: 12px 0 0;">
                        {{ __('Paiement à la livraison · Livraison offerte') }}
                    </p>
                </div>
            </div>
        </div>

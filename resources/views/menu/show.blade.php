...
@section('content')
    ...
    <a href="{{ route('menu.index') }}" class="btn btn-ghost btn-sm" style="margin-bottom: 18px;">
        <x-icon name="arrow-left" size="16" /> {{ __('Retour au menu') }}
    </a>
    ...
    <div class="detail-grid">
        ...
        <div>
            <div class="dish-cat" style="margin-bottom: 8px;">{{ $plat->categorie->nom ?? __('Plat') }}</div>
            <h1 style="margin: 0 0 10px; font-size: 30px; font-weight: 800; letter-spacing: -.03em;">{{ $plat->nom }}</h1>

            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">
                <span class="price" style="font-size: 24px;">{{ number_format($plat->prix, 2, ',', ' ') }} <small>{{ __('DH') }}</small></span>
                @if ($epuise)
            ...
            <div class="spec-list">
                <div class="spec">
                    ...
                    <div class="spec-lbl">{{ __('Préparation') }}</div>
                </div>
                <div class="spec">
            ...
            @if ($ingredients->isNotEmpty())
                <h3 style="font-size: 14px; font-weight: 700; margin: 4px 0 0;">{{ __('Ingrédients principaux') }}</h3>
            ...
            <div style="margin-top: 24px;">
                @if ($epuise)
                    <button class="btn btn-ghost btn-block" disabled>{{ __('Actuellement indisponible') }}</button>
                @else
                    <form method="POST" action="{{ route('panier.store', $plat) }}"
                        ...
                        <button type="submit" class="btn btn-primary" style="flex: 1; justify-content: center;">
                            <x-icon name="cart" size="17" /> {{ __('Ajouter au panier') }}
                        </button>
                    </form>
                @endif
            </div>
    ...
    @if ($similaires->isNotEmpty())
        <div class="section-head">
            <h2>{{ __('Dans la même catégorie') }}</h2>

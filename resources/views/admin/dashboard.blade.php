...
@section('crumb', 'Tableau de bord')

@section('content')
    <div class="page-head">
        <div class="page-titles">
            <h1>Tableau de bord</h1>
            <p>Vue d'ensemble de l'activité du restaurant — chiffre d'affaires et plats populaires.</p>
        </div>
    ...
    <div class="stat-grid">
        <div class="stat-card ink">
            <div class="stat-top">
                <span class="stat-lbl">Chiffre d'affaires du jour</span>
                <span class="stat-ico"><x-icon name="coins" size="20" /></span>
            </div>
            <div class="stat-val">{{ number_format($stats['ca_jour'], 2, ',', ' ') }} <small style="font-size: 14px;">DH</small></div>
            <div class="stat-lbl">CA total cumulé : {{ number_format($stats['ca_total'], 0, ',', ' ') }} DH</div>
        </div>
        ...
        <div class="stat-card">
            ...
            <div class="stat-lbl">{{ $parStatut[\App\Models\Commande::STATUT_PREPARATION] ?? 0 }} en préparation</div>
        </div>

        <div class="stat-card">
            <div class="stat-top">
                <span class="stat-lbl">Plats à la carte</span>
                <span class="stat-ico"><x-icon name="utensils" size="20" /></span>
            </div>
            <div class="stat-val">{{ $stats['plats'] }}</div>
            <div class="stat-lbl">{{ $stats['plats_epuises'] }} épuisé{{ $stats['plats_epuises'] > 1 ? 's' : '' }}</div>
        </div>
        ...
        <div class="stat-card">
            ...
            <div class="stat-lbl">{{ $stats['categories'] }} catégories</div>
        </div>
    </div>
    ...
    <div class="dash-grid">
        <div class="panel">
            <div class="panel-head">
                ...
                <span class="badge badge-neutral">DH</span>
            </div>
        ...
        <div class="panel">
            ...
            @if ($topPlats->isEmpty())
                <p class="muted" style="font-size: 13px;">Aucune commande enregistrée pour l'instant.</p>
            @else
                <div class="rank-list">
                    @foreach ($topPlats as $i => $plat)
                        <div class="rank-row">
                            ...
                            <span class="rank-val">{{ $plat->total_quantite }} vendus · {{ number_format($plat->total_ca, 0, ',', ' ') }} DH</span>
                        </div>
                    @endforeach
                </div>
            @endif
    ...
    <div class="panel" style="margin-top: 18px;">
        <div class="panel-head">
            ...
            <a href="{{ route('admin.commandes.index') }}" class="btn btn-ghost btn-sm">Tout voir <x-icon name="arrow-right" size="14" /></a>
        </div>
        @if ($recentes->isEmpty())
            <p class="muted" style="font-size: 13px;">Aucune commande pour le moment.</p>
        @else
            <div class="table-wrap" style="box-shadow: none; border-color: var(--border-soft);">
                <table class="data-table">
                    <thead>
                        <tr><th>#</th><th>Client</th><th>Date</th><th>Montant</th><th>Statut</th><th></th></tr>
                    </thead>
                    <tbody>
                        @foreach ($recentes as $commande)
                            <tr>
                                ...
                                <td class="cell-actions">
                                    <a href="{{ route('admin.commandes.show', $commande) }}" class="ghost-icon" aria-label="Voir"><x-icon name="eye" size="17" /></a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
